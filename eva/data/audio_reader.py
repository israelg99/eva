...
def select_generator(set_name, random_train_batches, full_sequences, fragment_length, batch_size, fragment_stride,
                     nb_output_bins, randomize_batch_order):
    if random_train_batches and set_name == 'train':
        bg = random_batch_generator
    else:
        bg = batch_generator
    return bg(full_sequences, fragment_length, batch_size, fragment_stride, nb_output_bins, randomize_batch_order)


def batch_generator(full_sequences, fragment_length, batch_size, fragment_stride, nb_output_bins, randomize_batch_order):
...
def random_batch_generator(full_sequences, fragment_length, batch_size, fragment_stride, nb_output_bins,
    ...
    while True:
        sequence_indices = np.random.randint(0, nb_sequences, size=batch_size)
        batch_inputs = []
        batch_outputs = []
        for i, seq_i in enumerate(sequence_indices):
            l = lengths[seq_i]
            offset = np.squeeze(np.random.randint(0, l - fragment_length, size=1))
...
def generators(dirname, desired_sample_rate, fragment_length, batch_size, fragment_stride, nb_output_bins,
               learn_all_outputs, use_ulaw, randomize_batch_order, random_train_batches):
    fragment_generators = {}
    nb_examples = {}
...
def generators_vctk(dirname, desired_sample_rate, fragment_length, batch_size, fragment_stride, nb_output_bins,
                    learn_all_outputs, use_ulaw, test_factor, randomize_batch_order, random_train_batches):
    fragment_generators = {}
    nb_examples = {}
